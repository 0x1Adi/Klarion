...
- **Offline heuristic.** A no-network `Verifier` (scores structure/placeholder signals) used when `mode = "off"`, or `mode = "auto"` with no credentials. Guarantees Klarion always produces verdicts. `mode = "on"` requires a real provider and errors if none can be constructed.
