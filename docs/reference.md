...
Use these hooks when the repo already runs [pre-commit](https://pre-commit.com): `klarion
protect` installs its own `.git/hooks/pre-commit`, so it refuses to overwrite pre-commit's,
and `--force` would delete it.
...
- **They ship off.** The manifest sets `stages: [manual]`, so adding this repo to a config
  blocks nobody's commits and builds nothing until someone adds `stages: [pre-commit]`.
  Until then the scan runs when you ask: `pre-commit run --hook-stage manual klarion`. That
  run checks staged content too; for the whole tree use `klarion scan .`.
- Everyone who commits needs an AI provider. Without one the hook exits 2 and the commit
  stops, which is why it ships off. `SKIP=klarion git commit` skips it once.
...
- **installs the scanner matching the release you pinned**, by tag or commit
  SHA: `@v0.4.1` runs the v0.4.1 binary, and `@v0` runs the release `v0`
  points to.
